...
HTTP 400 (version conflict). This simulates a service that races with
concurrent config changes and handles the conflict gracefully.
...
Environment:
    ...
    AUTH  — user:password (default: admin:adminpwd)
"""
...
AUTH = os.environ.get("AUTH", "admin:adminpwd")
...
def register(address, port, server_name, backend):
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        ...
        if commit_status == 400:
            # Version conflict on commit — someone else changed config.
            print(f"  commit returned 400 (version conflict) — retrying in {RETRY_DELAY}s")
            time.sleep(RETRY_DELAY)
            continue
